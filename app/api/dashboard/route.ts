...
export async function GET() {
  try {
    ...
    const uniqueCalled = new Set(calls.map((c: any) => c.patient_id)).size;
    const verifiedBookings = bookings.filter((b: any) => b.booking_status === 'verified').length;
    const selfReportedBookings = bookings.filter((b: any) => b.booking_status === 'self_reported').length;
    const attended = bookings.filter((b: any) => b.attendance_status === 'attended').length;

    return NextResponse.json({
      me: user,
      metrics: {
        patients: patients.length,
        unassigned: staffScope ? 0 : patients.filter((p: any) => p.assignment_status === 'unassigned').length,
        assigned: patients.filter((p: any) => p.assigned_to).length,
        uniqueCalled,
        verifiedBookings,
        selfReportedBookings,
        attended,
        openFlags: activeFlags.length
      },
      flags: activeFlags.slice(0, 50),
      recentUploads: staffScope ? [] : [...store.upload_batches].sort((a: any, b: any) => String(b.created_at).localeCompare(String(a.created_at))).slice(0, 5)
    });
  } catch (error: any) {
